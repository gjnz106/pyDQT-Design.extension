...
class SheetListTab(object):
    ...
    def _create_datagrid(self):
        ...
        grid.Columns.Add(DataGridHelper.create_text_column("Views", "views_on_sheet", 250))
    ...
    def on_create_click(self, sender, args):
        """Create new sheet(s) with dialog - supports manual and Excel import"""
        from System.Windows import MessageBox, MessageBoxButton, MessageBoxImage, MessageBoxResult, Window
        from System.Windows.Controls import Label, TextBox, Button, ComboBox, CheckBox, StackPanel, Grid, RowDefinition, RadioButton, GroupBox
        ...
        result = [False, "", "", None, False, 1, False, None]  # [result, number, name, titleblock_id, is_batch, count, is_excel, excel_path]
        ...
        dialog.Height = 550
        main_grid.RowDefinitions.Add(RowDefinition(Height=GridLength(80)))   # Mode selection
        ...
        main_grid.RowDefinitions.Add(RowDefinition(Height=GridLength(60)))   # Batch/Excel panel
        main_grid.RowDefinitions.Add(RowDefinition(Height=GridLength(80)))   # Count/Excel file
        main_grid.RowDefinitions.Add(RowDefinition(Height=GridLength(50)))   # Buttons
        
        # Mode Selection (Manual vs Excel)
        mode_group = GroupBox()
        mode_group.Header = "Creation Method"
        Grid.SetRow(mode_group, 0)
        
        mode_panel = StackPanel()
        mode_panel.Margin = Thickness(10, 5, 10, 5)
        mode_panel.Orientation = System.Windows.Controls.Orientation.Horizontal
        
        method_manual = RadioButton()
        method_manual.Content = "Manual Entry"
        method_manual.IsChecked = True
        method_manual.Margin = Thickness(0, 0, 30, 0)
        mode_panel.Children.Add(method_manual)
        
        method_excel = RadioButton()
        method_excel.Content = "From Excel File"
        method_excel.Margin = Thickness(0, 0, 0, 0)
        mode_panel.Children.Add(method_excel)
        
        mode_group.Content = mode_panel
        main_grid.Children.Add(mode_group)
        ...
        Grid.SetRow(tb_combo, 3)
        main_grid.Children.Add(tb_combo)
        
        # Manual: Batch create checkbox
        ...
        Grid.SetRow(batch_check, 4)
        main_grid.Children.Add(batch_check)
        
        # Excel: File picker panel
        excel_file_panel = StackPanel()
        excel_file_panel.Margin = Thickness(0, 10, 0, 0)
        excel_file_panel.Visibility = System.Windows.Visibility.Collapsed
        Grid.SetRow(excel_file_panel, 4)
        
        excel_info = Label()
        excel_info.Content = "Excel Format: Column A = Sheet Number, Column B = Sheet Name"
        excel_info.Foreground = System.Windows.Media.Brushes.Gray
        excel_info.FontSize = 10
        excel_file_panel.Children.Add(excel_info)
        
        main_grid.Children.Add(excel_file_panel)
        
        # Row 5 Container: Switches between Manual count controls and Excel file picker
        row5_container = Grid()
        Grid.SetRow(row5_container, 5)
        main_grid.Children.Add(row5_container)
        
        # Manual: Count (only enabled when batch checked)
        manual_count_panel = StackPanel()
        manual_count_panel.Visibility = System.Windows.Visibility.Visible  # Default: Manual mode visible
        
        count_label = Label()
        count_label.Content = "Number of sheets:"
        count_label.IsEnabled = False
        manual_count_panel.Children.Add(count_label)
        
        count_box = TextBox()
        count_box.Margin = Thickness(0, 25, 0, 0)
        count_box.Text = "10"
        count_box.IsEnabled = False
        manual_count_panel.Children.Add(count_box)
        
        row5_container.Children.Add(manual_count_panel)
        
        # Excel: File picker + status container
        excel_container = StackPanel()
        excel_container.Visibility = System.Windows.Visibility.Collapsed  # Hidden by default
        
        excel_picker_panel = StackPanel()
        excel_picker_panel.Orientation = System.Windows.Controls.Orientation.Horizontal
        excel_picker_panel.Margin = Thickness(0, 15, 0, 0)
        
        file_label = Label()
        file_label.Content = "Excel File:"
        file_label.Width = 80
        excel_picker_panel.Children.Add(file_label)
        
        file_path_box = TextBox()
        file_path_box.Width = 240
        file_path_box.IsReadOnly = True
        file_path_box.Background = System.Windows.Media.Brushes.LightGray
        excel_picker_panel.Children.Add(file_path_box)
        
        # Browse Excel file function - DEFINE BEFORE creating button
        def on_browse(s, e):
            from System.Windows.Forms import OpenFileDialog, DialogResult
            
            file_dialog = OpenFileDialog()
            file_dialog.Filter = "Excel Files (*.xlsx;*.xls)|*.xlsx;*.xls"
            file_dialog.Title = "Select Excel File with Sheet List"
            
            if file_dialog.ShowDialog() == DialogResult.OK:
                file_path_box.Text = file_dialog.FileName
                result[7] = file_dialog.FileName
                ...
                try:
                    ...
                    sheet_list = []
                    with zipfile.ZipFile(file_dialog.FileName, 'r') as zip_file:
                        # Read shared strings
                        shared_strings = []
                        try:
                            with zip_file.open('xl/sharedStrings.xml') as f:
                                ...
                                for si in root.findall('x:si', ns):
                                    text_elem = si.find('x:t', ns)  # Renamed from 't'
                        ...
                        with zip_file.open('xl/worksheets/sheet1.xml') as f:
                            ...
                            for row in root.findall('.//x:row', ns):
                                ...
                                if len(cells) >= 2:
                                    ...
                                    if len(values) == 2:
                                        sheet_list.append((values[0].strip(), values[1].strip()))
                    
                    if sheet_list:
                        excel_status.Content = "{} sheets loaded from Excel".format(len(sheet_list))
                        excel_status.Foreground = System.Windows.Media.Brushes.Green
                    else:
                        excel_status.Content = "No valid data found in Excel"
                        excel_status.Foreground = System.Windows.Media.Brushes.Red
                        result[7] = None
                
                except Exception as ex:
                    excel_status.Content = "Error reading file: {}".format(str(ex))
                    excel_status.Foreground = System.Windows.Media.Brushes.Red
                    result[7] = None
        ...
        browse_btn.Width = 80
        browse_btn.Height = 25
        browse_btn.Margin = Thickness(10, 0, 0, 0)
        browse_btn.IsEnabled = True  # Explicitly enable
        browse_btn.Click += on_browse  # Wire event IMMEDIATELY
        excel_picker_panel.Children.Add(browse_btn)
        
        excel_container.Children.Add(excel_picker_panel)
        
        # Excel status label
        excel_status = Label()
        excel_status.Content = ""
        excel_status.Foreground = System.Windows.Media.Brushes.Gray
        excel_status.FontSize = 10
        excel_status.Margin = Thickness(0, 10, 0, 0)
        excel_container.Children.Add(excel_status)
        
        row5_container.Children.Add(excel_container)
        
        # Toggle controls based on mode
        def on_mode_changed(s, e):
            if method_manual.IsChecked:
                # Show manual controls
                number_label.Visibility = System.Windows.Visibility.Visible
                number_box.Visibility = System.Windows.Visibility.Visible
                name_label.Visibility = System.Windows.Visibility.Visible
                name_box.Visibility = System.Windows.Visibility.Visible
                batch_check.Visibility = System.Windows.Visibility.Visible
                manual_count_panel.Visibility = System.Windows.Visibility.Visible
                excel_file_panel.Visibility = System.Windows.Visibility.Collapsed
                excel_container.Visibility = System.Windows.Visibility.Collapsed
            else:
                # Show Excel controls
                number_label.Visibility = System.Windows.Visibility.Collapsed
                number_box.Visibility = System.Windows.Visibility.Collapsed
                name_label.Visibility = System.Windows.Visibility.Collapsed
                name_box.Visibility = System.Windows.Visibility.Collapsed
                batch_check.Visibility = System.Windows.Visibility.Collapsed
                manual_count_panel.Visibility = System.Windows.Visibility.Collapsed
                excel_file_panel.Visibility = System.Windows.Visibility.Visible
                excel_container.Visibility = System.Windows.Visibility.Visible
        
        method_manual.Checked += on_mode_changed
        method_excel.Checked += on_mode_changed
        ...
        btn_panel.HorizontalAlignment = System.Windows.HorizontalAlignment.Right
        Grid.SetRow(btn_panel, 6)  # Changed to row 6
        
        def on_ok(s, e):
            result[0] = True
            result[6] = method_excel.IsChecked == True  # Is Excel mode
            
            if result[6]:  # Excel mode
                if not result[7]:  # No Excel file
                    MessageBox.Show("Please select an Excel file first", "Error",
            ...
            result[3] = titleblocks[tb_combo.SelectedIndex][0]  # Titleblock always needed
            dialog.Close()
        ...
        try:
            if result[6]:  # Excel mode
                ...
                # Read Excel file
                sheet_list = []
                with zipfile.ZipFile(result[7], 'r') as zip_file:
                    # Read shared strings
                    shared_strings = []
                    try:
                        with zip_file.open('xl/sharedStrings.xml') as f:
                            ...
                            ns = {'x': 'http://schemas.openxmlformats.org/spreadsheetml/2006/main'}
                            for si in root.findall('x:si', ns):
                                text_elem = si.find('x:t', ns)  # Renamed from 't' to avoid collision!
                                if text_elem is not None:
                                    shared_strings.append(text_elem.text)
                    except:
                        pass
                ...
                success_count = 0
                errors = []
                for sheet_num, sheet_name in sheet_list:
                    try:
                        new_sheet = self.revit_service.create_sheet(sheet_num, sheet_name, result[3])
                        if new_sheet:
                            success_count += 1
                        else:
                            errors.append("{}: Failed to create".format(sheet_num))
                    except Exception as e:
                        errors.append("{}: {}".format(sheet_num, str(e)))
                
                t.Commit()
                self.refresh_data()
                
                # Show results
                msg = "Created {} sheet(s) successfully".format(success_count)
                if errors:
                ...
                MessageBox.Show(msg, "Results", MessageBoxButton.OK, MessageBoxImage.Information)
                
            elif result[4]:  # Manual batch mode
                success_count = 0
